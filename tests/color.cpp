...
TEST_CASE("Color is valid hex color pass valid") {
    std::string hex_color = "#0fEdb3";
    int is_valid = is_valid_hex_color(hex_color);
    REQUIRE(is_valid);
}

TEST_CASE("Color is valid hex color pass bad length") {
    std::string hex_color = "#1234567";
    int is_valid = is_valid_hex_color(hex_color);
    CHECK_FALSE(is_valid);
}

TEST_CASE("Color is valid hex color pass no hash prefix") {
...
TEST_CASE("Color get scaled value") {
